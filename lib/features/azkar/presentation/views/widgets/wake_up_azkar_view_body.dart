...
class WakeUpAzkarViewBody extends StatefulWidget {
  const WakeUpAzkarViewBody({super.key});

  @override
  State<WakeUpAzkarViewBody> createState() => _WakeUpAzkarViewBodyState();
}

class _WakeUpAzkarViewBodyState extends State<WakeUpAzkarViewBody> {
  @override
  void initState() {
    super.initState();
    BlocProvider.of<GetAzkarCubit>(context).getAzkarCubit(categoryAzkarCubit: 'wake_up_azkar');
    setState(() {});
  }
    ...
    return BlocBuilder<GetAzkarCubit, GetAzkarStates>(
      builder: (context, state) {
        if (state is AzkarFaliure) {
          return const Center(child: Text('No Wake Up Azkar Now'));
        }
